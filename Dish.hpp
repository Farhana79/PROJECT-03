...
public:
     ...
     * Two dishes are equal if they have the same name, same cuisine type, same preparation time,
     * and the same price.
     */
    ...
    /**
     * @param : A const reference to the right-hand side of the `!=` operator.
     * @return : Returns true if the right-hand side dish is NOT "equal" (`!=`), false otherwise.
     * Two dishes are NOT equal if any of their name, cuisine type, preparation time, or price are not equal.
     */
     bool operator!=(const Dish& rhs) const;
